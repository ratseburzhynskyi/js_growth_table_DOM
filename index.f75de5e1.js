 ...
 */ const getTbody = ()=>{
    if (field.tBodies.length) return field.tBodies[0];
 ...
 */ const renderTable = ()=>{
    ...
    for(let r = 0; r < rows; r++){
        ...
        for(let c = 0; c < columns; c++)tr.appendChild(document.createElement("td"));
...
/* ---------- INIT ---------- */ renderTable();
updateButtons();
/* ---------- ROWS ---------- */ appendRowButton.addEventListener("click", ()=>{
...
/* ---------- COLUMNS ---------- */ appendColumnButton.addEventListener("click", ()=>{
    if (columns === MAX_SIZE) return;
    const tbody = getTbody();
    for (const row of tbody.rows)row.appendChild(document.createElement("td"));
    columns++;
    updateButtons();
});
removeColumnButton.addEventListener("click", ()=>{
    if (columns === MIN_SIZE) return;
    const tbody = getTbody();
    for (const row of tbody.rows)row.lastElementChild.remove();
    columns--;
    updateButtons();
});
